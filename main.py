...
def Lagrandj(Xisc, i):
    global x, y
    Yisc1 = (4 * pow(Xisc, 3) - 3 * x[1] * pow(Xisc, 2) - 3 * x[2] * pow(Xisc, 2) - 3 * x[3] * pow(Xisc, 2) - 3 * x[
        4] * pow(Xisc, 2) + 2 * x[1] * x[2] * Xisc + 2 * x[1] * x[3] * Xisc + 2 * x[1] * x[4] * Xisc + 2 * x[2] * x[
                 3] * Xisc + 2 * x[2] * x[4] * Xisc + 2 * x[3] * x[4] * Xisc - x[1] * x[2] * x[3] - x[1] * x[2] * x[4] -
             x[1] * x[3] * x[4] - x[2] * x[3] * x[4])
    Yisc1 *= y[0]
    Yisc1 /= ((x[0] - x[1]) * (x[0] - x[2]) * (x[0] - x[3]) * (x[0] - x[4]))
    Yisc2 = (4 * pow(Xisc, 3) - 3 * x[0] * pow(Xisc, 2) - 3 * x[2] * pow(Xisc, 2) - 3 * x[3] * pow(Xisc, 2) - 3 * x[
        4] * pow(Xisc, 2) + 2 * x[0] * x[2] * Xisc + 2 * x[0] * x[3] * Xisc + 2 * x[0] * x[4] * Xisc + 2 * x[2] * x[
                 3] * Xisc + 2 * x[2] * x[4] * Xisc + 2 * x[3] * x[4] * Xisc - x[0] * x[2] * x[3] - x[0] * x[2] * x[4] -
             x[0] * x[3] * x[4] - x[2] * x[3] * x[4])
    Yisc2 *= y[1]
    Yisc2 /= ((x[1] - x[0]) * (x[1] - x[2]) * (x[1] - x[3]) * (x[1] - x[4]))
    Yisc3 = (4 * pow(Xisc, 3) - 3 * x[0] * pow(Xisc, 2) - 3 * x[1] * pow(Xisc, 2) - 3 * x[3] * pow(Xisc, 2) - 3 * x[
        4] * pow(Xisc, 2) + 2 * x[0] * x[1] * Xisc + 2 * x[0] * x[3] * Xisc + 2 * x[0] * x[4] * Xisc + 2 * x[1] * x[
                 3] * Xisc + 2 * x[1] * x[4] * Xisc + 2 * x[3] * x[4] * Xisc - x[0] * x[1] * x[3] - x[0] * x[1] * x[4] -
             x[0] * x[3] * x[4] - x[1] * x[3] * x[4])
    Yisc3 *= y[2]
    Yisc3 /= ((x[2] - x[0]) * (x[2] - x[1]) * (x[2] - x[3]) * (x[2] - x[4]))
    Yisc4 = (4 * pow(Xisc, 3) - 3 * x[0] * pow(Xisc, 2) - 3 * x[1] * pow(Xisc, 2) - 3 * x[2] * pow(Xisc, 2) - 3 * x[
        4] * pow(Xisc, 2) + 2 * x[0] * x[1] * Xisc + 2 * x[0] * x[2] * Xisc + 2 * x[0] * x[4] * Xisc + 2 * x[1] * x[
                 2] * Xisc + 2 * x[1] * x[4] * Xisc + 2 * x[2] * x[4] * Xisc - x[0] * x[1] * x[2] - x[0] * x[1] * x[4] -
             x[0] * x[2] * x[4] - x[1] * x[2] * x[4])
    Yisc4 *= y[3]
    Yisc4 /= ((x[3] - x[0]) * (x[3] - x[1]) * (x[3] - x[2]) * (x[3] - x[4]))
    Yisc5 = (4 * pow(Xisc, 3) - 3 * x[0] * pow(Xisc, 2) - 3 * x[1] * pow(Xisc, 2) - 3 * x[2] * pow(Xisc, 2) - 3 * x[
        3] * pow(Xisc, 2) + 2 * x[0] * x[1] * Xisc + 2 * x[0] * x[2] * Xisc + 2 * x[0] * x[3] * Xisc + 2 * x[1] * x[
                 2] * Xisc + 2 * x[1] * x[3] * Xisc + 2 * x[2] * x[3] * Xisc - x[0] * x[1] * x[2] - x[0] * x[1] * x[3] -
             x[0] * x[2] * x[3] - x[1] * x[2] * x[3])
    Yisc5 *= y[4]
    Yisc5 /= ((x[4] - x[0]) * (x[4] - x[1]) * (x[4] - x[2]) * (x[4] - x[3]))
    # Yisc1 = (4*pow(Xisc, 3)-3*x[i+1]*pow(Xisc, 2)-3*x[2]*pow(Xisc, 2)-3*x[3]*pow(Xisc, 2)-3*x[4]*pow(Xisc, 2)+2*x[i+1]*x[2]*Xisc+2*x[i+1]*x[3]*Xisc+2*x[i+1]*x[4]*Xisc+2*x[2]*x[3]*Xisc+2*x[2]*x[4]*Xisc+2*x[3]*x[4]*Xisc-x[i+1]*x[2]*x[3]-x[i+1]*x[2]*x[4]-x[i+1]*x[3]*x[4]-x[2]*x[3]*x[4])
    # Yisc1 *= y[0]
    # Yisc1 /= ((x[0]-x[i+1])*(x[0]-x[2])*(x[0]-x[3])*(x[0]-x[4]))
    ...
    # Yisc2 *= y[1]
    # # Yisc2 /= ((x[i+1]-x[0])*(x[i+1]-x[2])*(x[i+1]-x[3])*(x[i+1]-x[4]))
    # Yisc3 = (4*pow(Xisc, 3)-3*x[0]*pow(Xisc, 2)-3*x[i+1]*pow(Xisc, 2)-3*x[3]*pow(Xisc, 2)-3*x[4]*pow(Xisc, 2)+2*x[0]*x[i+1]*Xisc+2*x[0]*x[3]*Xisc+2*x[0]*x[4]*Xisc+2*x[i+1]*x[3]*Xisc+2*x[i+1]*x[4]*Xisc+2*x[3]*x[4]*Xisc-x[0]*x[i+1]*x[3]-x[0]*x[i+1]*x[4]-x[0]*x[3]*x[4]-x[i+1]*x[3]*x[4])
    # Yisc3 *= y[2]
    # Yisc3 /= ((x[2]-x[0])*(x[2]-x[i+1])*(x[2]-x[3])*(x[2]-x[4]))
    # Yisc4 = (4*pow(Xisc, 3)-3*x[0]*pow(Xisc, 2)-3*x[i+1]*pow(Xisc, 2)-3*x[2]*pow(Xisc, 2)-3*x[4]*pow(Xisc, 2)+2*x[0]*x[i+1]*Xisc+2*x[0]*x[2]*Xisc+2*x[0]*x[4]*Xisc+2*x[i+1]*x[2]*Xisc+2*x[i+1]*x[4]*Xisc+2*x[2]*x[4]*Xisc-x[0]*x[i+1]*x[2]-x[0]*x[i+1]*x[4]-x[0]*x[2]*x[4]-x[i+1]*x[2]*x[4])
    # Yisc4 *= y[3]
    # Yisc4 /= ((x[3]-x[0])*(x[3]-x[i+1])*(x[3]-x[2])*(x[3]-x[4]))
    # Yisc5 = (4*pow(Xisc, 3)-3*x[0]*pow(Xisc, 2)-3*x[i+1]*pow(Xisc, 2)-3*x[2]*pow(Xisc, 2)-3*x[3]*pow(Xisc, 2)+2*x[0]*x[i+1]*Xisc+2*x[0]*x[2]*Xisc+2*x[0]*x[3]*Xisc+2*x[i+1]*x[2]*Xisc+2*x[i+1]*x[3]*Xisc+2*x[2]*x[3]*Xisc-x[0]*x[i+1]*x[2]-x[0]*x[i+1]*x[3]-x[0]*x[2]*x[3]-x[i+1]*x[2]*x[3])
    # Yisc5 *= y[4]
    # Yisc5 /= ((x[4]-x[0])*(x[4]-x[i+1])*(x[4]-x[2])*(x[4]-x[3]))
...
def Test():
    ...
    Yj = []  # Третьи разности
    for i in range(len(x)):
        Yj.append(((2 * x[i] - x[1] - x[2]) * y[0]) / ((x[0] - x[1]) * (x[0] - x[2])) + (
                ((2 * x[i] - x[0] - x[2]) * y[1]) / ((x[1] - x[0]) * (x[1] - x[2])) + (
                ((2 * x[i] - x[0] - x[1]) * y[2]) / ((x[2] - x[0]) * (x[2] - x[1])))))
...
def formula():
    ...
    fin += ["Пятые разности: "]
    fin += [
        r"$P^{'}(x)=\frac{(4x^{3}-3x_{1}x^{2}-3x_{2}x^{2}-3x_{3}x^{2}-3x_{4}x^{2}+2x_{1}x_{2}x+2x_{1}x_{3}x+2x_{1}x_{4}x+2x_{2}x_{3}x+2x_{2}x_{4}x+2x_{3}x_{4}x-x_{1}x_{2}x_{3}-x_{1}x_{2}x_{4}-x_{1}x_{3}x_{4}-x_{2}x_{3}x_{4})y_{0}}{(x_{0}-x_{1})(x_{0}-x_{2})(x_{0}-x_{3})(x_{0}-x_{4})}$"]
    fin += [
        r"$+\frac{(4x^{3}-3x_{0}x^{2}-3x_{2}x^{2}-3x_{3}x^{2}-3x_{4}x^{2}+2x_{0}x_{2}x+2x_{0}x_{3}x+2x_{0}x_{4}x+2x_{2}x_{3}x+2x_{2}x_{4}x+2x_{3}x_{4}x-x_{0}x_{2}x_{3}-x_{0}x_{2}x_{4}-x_{0}x_{3}x_{4}-x_{2}x_{3}x_{4})y_{1}}{(x_{1}-x_{0})(x_{1}-x_{2})(x_{1}-x_{3})(x_{1}-x_{4})}$"]
    fin += [
        r"$+\frac{(4x^{3}-3x_{0}x^{2}-3x_{1}x^{2}-3x_{3}x^{2}-3x_{4}x^{2}+2x_{0}x_{1}x+2x_{0}x_{3}x+2x_{0}x_{4}x+2x_{1}x_{3}x+2x_{1}x_{4}x+2x_{3}x_{4}x-x_{0}x_{1}x_{3}-x_{0}x_{1}x_{4}-x_{0}x_{3}x_{4}-x_{1}x_{3}x_{4})y_{2}}{(x_{2}-x_{0})(x_{2}-x_{1})(x_{2}-x_{3})(x_{2}-x_{4})}$"]
    fin += [
        r"$+\frac{(4x^{3}-3x_{0}x^{2}-3x_{1}x^{2}-3x_{2}x^{2}-3x_{4}x^{2}+2x_{0}x_{1}x+2x_{0}x_{2}x+2x_{0}x_{4}x+2x_{1}x_{2}x+2x_{1}x_{4}x+2x_{2}x_{4}x-x_{0}x_{1}x_{2}-x_{0}x_{1}x_{4}-x_{0}x_{2}x_{4}-x_{1}x_{2}x_{4})y_{3}}{(x_{3}-x_{0})(x_{3}-x_{1})(x_{3}-x_{2})(x_{3}-x_{4})}$"]
    fin += [
        r"$+\frac{(4x^{3}-3x_{0}x^{2}-3x_{1}x^{2}-3x_{2}x^{2}-3x_{3}x^{2}+2x_{0}x_{1}x+2x_{0}x_{2}x+2x_{0}x_{3}x+2x_{1}x_{2}x+2x_{1}x_{3}x+2x_{2}x_{3}x-x_{0}x_{1}x_{2}-x_{0}x_{1}x_{3}-x_{0}x_{2}x_{3}-x_{1}x_{2}x_{3})y_{4}}{(x_{4}-x_{0})(x_{4}-x_{1})(x_{4}-x_{2})(x_{4}-x_{3})}$"]
    ...
    fin += ["Третьи разности: "]
    fin += [
        r"$P^{'}(x)=\frac{(2x-x_{1}-x_{2})y_{0}}{(x_{0}-x_{1})(x_{0}-x_{2})}+\frac{(2x-x_{0}-x_{2})y_{1}}{(x_{1}-x_{0})(x_{1}-x_{2})}+\frac{(2x-x_{0}-x_{1})y_{2}}{(x_{2}-x_{0})(x_{2}-x_{1})}$"]
    # h = x[1] - x[0]
